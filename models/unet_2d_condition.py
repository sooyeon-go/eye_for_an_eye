...
def matching_feature(x, mask_lst=None):
    feat_size = [32,64]
    match_dict={}
    for i in feat_size:
        if mask_lst:
            style_mask = F.interpolate(torch.tensor(mask_lst[0][0]).float().view(1,1,512,512), size=(i, i), mode='nearest').view(i, i).unsqueeze(-1).repeat(1, 1, x.shape[1]).to(x.device)
            struct_mask = F.interpolate(torch.tensor(mask_lst[1][0]).float().view(1,1,512,512), size=(i, i), mode='nearest').view(i, i).unsqueeze(-1).repeat(1, 1, x.shape[1]).to(x.device)
        ...
        x = nn.Upsample(size=(i, i), mode='bilinear')(x)
        out_x = x[0] #[c,h,w]
        style_x = x[1] 
        ...
        for x_coor, y_coor in np.ndindex(i, i):
            out_vec = out_x[:, y_coor, x_coor].view(1, -1)  # 1, C
            if torch.all(out_vec == 0):
                pass
            else:
                style_vec = rearrange(style_x, 'c h w -> c (h w)', h=i).unsqueeze(dim=0)
                
                out_vec = F.normalize(out_vec) # 1, C
                style_vec = F.normalize(style_vec) # 1, C, HW
                cos_map = torch.matmul(out_vec, style_vec).view(1, i, i).cpu().numpy() # N, H, W
        ...
        match_dict[f'{i}']=match_lst
        match_dict[f'src_{i}']=src_lst
...
class FreeUUNet2DConditionModel(UNet2DConditionModel):
    ...
    def forward(
            ...
            match_coord: Optional[List[torch.Tensor]] = None
