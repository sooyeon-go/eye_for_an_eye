...
from constants import STYLE_INDEX, STRUCT_INDEX, OUT_INDEX
...
class Segmentor:
    ...
    def aggregate_cross_attn_map(self, idx, thres, token_idx=5):
        ...
        image = torch.where(image > thres, torch.tensor(1), torch.tensor(0))
           
        return image
    
    def cross_attn_map(self, token_idx=5, thres=0.1):
        mask_style_32 = self.aggregate_cross_attn_map(STYLE_INDEX, thres=thres)
        mask_struct_32 = self.aggregate_cross_attn_map(STRUCT_INDEX, thres=thres)
        
        return mask_style_32, mask_struct_32
