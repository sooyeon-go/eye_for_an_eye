...
def should_mix_features_mix(model, hidden_states: torch.Tensor) -> bool:
    """ Verify whether we should perform the mixing in the current timestep. """
    # feature range = [8, 16, 32, 64]
    is_in_32_timestep_range = (
            model.config.feat_range.start <= model.step < model.config.feat_range.end
    )
    is_in_64_timestep_range = (
            model.config.feat_range.start <= model.step < model.config.feat_range.end
    )
